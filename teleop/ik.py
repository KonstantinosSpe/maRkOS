#!/usr/bin/env python3
"""
ik.py — the arm's geometry: link lengths, joint limits, and the 2-link IK
============================================================================
This is the pure-math half of the project — nothing in here touches a
serial port or a camera, so it's safe to import and test on its own.
...
# ── Speed ceilings + response-curve shape for gesture-driven targets ──────
# These describe how fast the *commanded target* (height/reach/twist) is
# allowed to move in gesture space — separate from HW_MAX_SPEED in
# hw_config.py, which caps how fast the motors are actually told to turn
# once that target has been converted into joint angles.
MAX_J1_SPEED = 80.0
MAX_HEIGHT_SPEED = 160.0
MAX_REACH_SPEED = 140.0
DEADZONE = 0.04
CURVE_POW = 2.0
REACH_GAIN = 4.0   # halved from an earlier 8.0 — depth (how big the hand
                    # looks to the camera) is a touchy signal to begin with,
                    # since perspective makes a small real forward/back
                    # movement swing it a lot. At 8.0 it was reaching full
                    # speed for noticeably less physical movement than
                    # height needed, so depth felt like it was fighting the
                    # other two axes for attention.
...
def response(x: float) -> float:
    """Turn a raw offset in [-1, 1] into an output speed fraction, with a
    small dead zone near zero and a curved (not linear) ramp past it — so
    a small intentional movement still reads as deliberate, but the very
    center, where your hand naturally drifts a little even when you mean
    to hold still, doesn't twitch the arm."""
    s = abs(x)
    if s < DEADZONE:
        return 0.0
